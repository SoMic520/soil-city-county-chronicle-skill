...
LOCKED_GROUPS = ('styles', 'execution', 'county_requirements')
...
def validate_layout(layout, baseline, final=False):
    ...
    toc_max = actual.get('county_requirements.toc_max_depth')
    if type(toc_max) is not int or not 1 <= toc_max <= len(toc_roles):
        issue('LAYOUT_TOC', 'county_requirements.toc_max_depth', '目录最大深度应为1至5的整数；偏离N1的2还须有效逐字段例外')
        toc_max = baseline['county_requirements']['toc_max_depth']
