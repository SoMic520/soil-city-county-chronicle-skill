"""Synthetic regression fixtures: no real municipal/county facts or services."""
import csv
import json
import tempfile
import unittest
...
class ProjectTests(unittest.TestCase):
    def setUp(self):
        ...
        self.project['project'].update(administrative_unit='合成县', scope='合成评价范围', period='2023', standards_confirmed=True, standards_basis='合成任务确认记录')
        self.project['project']['administrative_level'].update(value='county', status='confirmed', basis='合成任务明确为县级成果')
    ...
    def complete_fixture(self):
        ...
        self.rows('soil_types.csv', [dict(type_id='T1', record_level='soil_species', soil_class='合成土类', subclass='合成亚类', genus='合成土属', name='合成土种', source_id='S1', locator='合成分类表', section_id='3', evidence_ids='E3', profile_kind='historical_description', profile_source_id='S1', profile_locator='合成历史记述', record_locator='第三章合成土种', status='verified')])
        self.save()
        ...
        layout['level_profile'].update(project='county', guide_profile='N1-county', basis='合成县级导引确认记录')
        ...
        length['level_profile'].update(project='county', template='county', basis='合成任务与模板均为县级')
        length['project_structure'].update(soil_narrative_units=10, soil_narrative_unit_kind='soil_species',
                                           soil_classes=1, soil_subgroups=1, soil_genera=1, soil_species=10,
                                           property_indicators=10, evaluation_modules=10,
                                           recommendation_topics=10, basis='合成模板及本项目结构清单')
        targets = []
    ...
    def test_municipal_profile_uses_soil_genus_and_matching_plans(self):
        self.complete_fixture()
        self.project['project']['administrative_unit'] = '合成市'
        self.project['project']['administrative_level'].update(value='municipal', status='confirmed', basis='合成市级任务书')
        self.project['soil_inventory']['expected_ids'] = ['G1']
        self.save()
        rows = c.table(self.folder, 'soil_types.csv', 'type_id')
        row = rows.pop('T1')
        row.update(type_id='G1', record_level='soil_genus', name='合成土属', profile_kind='not_required',
                   profile_source_id='', profile_locator='', profile_figure_ids='', record_locator='第三章合成土属')
        self.rows('soil_types.csv', [row])
        layout = json.loads((self.folder / 'layout.json').read_text(encoding='utf-8'))
        layout['level_profile'].update(project='municipal', guide_profile='N2-municipal', basis='合成市级导引', municipal_guide_confirmed=True)
        (self.folder / 'layout.json').write_text(json.dumps(layout, ensure_ascii=False), encoding='utf-8')
        length = json.loads((self.folder / 'length-plan.json').read_text(encoding='utf-8'))
        length['level_profile'].update(project='municipal', template='municipal', basis='合成市级模板')
        length['project_structure'].update(soil_narrative_unit_kind='soil_genus', soil_genera=10, soil_species=50)
        (self.folder / 'length-plan.json').write_text(json.dumps(length, ensure_ascii=False), encoding='utf-8')
        self.assertEqual(c.check_project(self.folder, final=True)['issues'], [])
