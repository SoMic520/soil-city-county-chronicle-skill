...
class LengthProfileTests(unittest.TestCase):
    ...
    def plan(self):
        ...
        rows = {}
        kinds = {'3':'soil_narrative_units','4':'property_indicators','5':'evaluation_modules','6':'recommendation_topics'}
        ...
        return {'length_schema_version':2, 'measurement_version':lp.VERSION, 'status':'locked',
                'level_profile':{'project':'county','template':'county','basis':'合成层级确认'},
                'template':{'path':'x','sha256':'0'*64,'identity':'合成模板','measurement_evidence':'合成测试','chapters':t},
                'project_structure':{'soil_narrative_units':10,'property_indicators':10,'evaluation_modules':10,'recommendation_topics':10,'basis':'合成结构清单'},
    ...
    def test_city_county_template_mismatch_is_blocked(self):
        p = self.plan(); p['level_profile']['project'] = 'municipal'
        self.assertIn('LENGTH_LEVEL_MISMATCH', self.codes(p))

    def test_calculate_targets_is_deterministic_and_never_locks(self):
        p = self.plan()
        p['project_structure']['soil_narrative_units'] = 20
    ...
    def test_variable_chapter_uses_structure_density(self):
        p = self.plan(); p['project_structure']['soil_narrative_units'] = 20
    ...
    def test_bad_types_fail_closed(self):
        ...
        p = self.plan(); p['project_structure']['soil_narrative_units'] = -1
