...
class LayoutTests(unittest.TestCase):
    ...
    def complete(self):
        ...
        self.plan['level_profile'].update(project='county', guide_profile='N1-county', basis='合成县级导引确认记录')
    ...
    def test_level_and_guide_must_match(self):
        self.plan['level_profile'].update(project='municipal', guide_profile='N1-county', basis='合成错误映射')
        self.assertIn('LAYOUT_LEVEL_MISMATCH', self.codes())
    ...
    def test_toc_depth_exception_requires_basis_and_valid_integer(self):
        ...
        self.plan['acceptance_requirements']['toc_max_depth'] = 3
        self.assertIn('LAYOUT_PROFILE_DRIFT', self.codes())
        self.override('acceptance_requirements.toc_max_depth', 3)
        self.assertEqual(self.codes(True), set())
        for value in [0,6,True,2.5]:
            self.plan['acceptance_requirements']['toc_max_depth'] = value
