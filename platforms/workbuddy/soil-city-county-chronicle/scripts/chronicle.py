...
def check_project(folder, final=False):
    ...
    p = data['project']
    required_project_fields = ('county', 'scope', 'period') if schema_version == 1 else ('administrative_unit', 'scope', 'period')
    for key in required_project_fields:
        if not p.get(key):
            issue('PROJECT', key, '项目范围字段未填写')
    administrative_level_value = 'county' if schema_version == 1 else 'unknown'
    if schema_version == 1:
        notes.append('旧项目schema_version=1未记录市县级证据；草稿检查兼容，终稿前须迁移到schema_version=2')
        if final:
            issue('LEVEL_SCHEMA', 'administrative_level', '终稿须迁移并确认市级或县级成果层级')
    else:
        level = p.get('administrative_level')
        if not isinstance(level, dict):
            raise ValueError('project.administrative_level必须为对象')
        if level.get('value') not in {'municipal', 'county', 'unknown'}:
            raise ValueError('administrative_level.value无效')
        administrative_level_value = level.get('value')
        if level.get('status') not in {'unconfirmed', 'detected', 'confirmed'}:
            raise ValueError('administrative_level.status无效')
        if level.get('value') == 'unknown':
            if final:
                issue('LEVEL_UNKNOWN', 'administrative_level', '终稿前须确认市级或县级成果层级')
            else:
                notes.append('市县级尚未确认；可继续共享资料工作，不锁定层级专属章节、篇幅和验收规则')
        elif level.get('status') != 'confirmed' or not str(level.get('basis', '')).strip():
            if final:
                issue('LEVEL_CONFIRMATION', 'administrative_level', '终稿前须记录层级确认状态和可定位依据')
            else:
                notes.append('已形成层级预判，终稿前须由项目依据确认并填写basis')
    topics = data['topics']
    ...
    if length_path.exists():
        ...
        length_project_level = length_plan.get('level_profile', {}).get('project') if isinstance(length_plan.get('level_profile'), dict) else None
        if administrative_level_value in {'municipal', 'county'} and length_project_level not in {None, 'unknown', administrative_level_value}:
            issue('LEVEL_LENGTH_MISMATCH', 'length-plan.json', '项目层级与篇幅计划登记层级不一致')
    elif final:
    ...
    if final:
        ...
        layout_project_level = layout.get('level_profile', {}).get('project') if isinstance(layout.get('level_profile'), dict) else None
        if administrative_level_value in {'municipal', 'county'} and layout_project_level != administrative_level_value:
            issue('LEVEL_LAYOUT_MISMATCH', 'layout.json', '项目层级与排版计划登记层级不一致')
...
def coverage_checks(folder, data, sections, topics, sources, evidence, figures, issue, notes, final):
    ...
    project_level = 'county' if data.get('schema_version') == 1 else data.get('project', {}).get('administrative_level', {}).get('value', 'unknown')
    expected_record_level = {'municipal': 'soil_genus', 'county': 'soil_species'}.get(project_level)
    ...
    if inventory_required or inventory.get('confirmed') is True:
        if inventory.get('confirmed') is not True or not ids or not inventory.get('locator'):
            issue('SOIL_INVENTORY', '3', '缺经核对的完整分类叙述单元清单和原表定位')
        else:
            source_ok(inventory.get('source_id', ''), 'soil_inventory')
        for tid in sorted(set(ids) - set(soil_types)):
            issue('SOIL_TYPE_MISSING', tid, '审定清单中的分类叙述单元没有记述覆盖记录')
        for tid in sorted(set(soil_types) - set(ids)):
            issue('SOIL_TYPE_EXTRA', tid, '覆盖记录不在确认的分类叙述单元清单中')
    for tid, t in soil_types.items():
        if t['status'] not in {'draft', 'verified', 'withheld'} or t['profile_kind'] not in {'third_survey', 'historical_profile', 'historical_description', 'missing', 'not_required'}:
            raise ValueError(f'invalid soil type status/profile_kind: {tid}')
        if expected_record_level and t.get('record_level') != expected_record_level:
            issue('SOIL_LEVEL', tid, f'{project_level}项目第三章记录层级应为{expected_record_level}')
        if t['section_id'] not in sections:
            issue('SOIL_SECTION', tid, '分类单元记述章节不存在')
        if inventory_required and t['status'] != 'verified':
            issue('SOIL_TYPE_PENDING', tid, '该分类单元记述尚未完成；不能省略')
        if t['status'] != 'verified':
            continue
        if not all(t[k] for k in ('soil_class', 'subclass', 'genus', 'name', 'source_id', 'locator', 'record_locator')):
            issue('SOIL_TYPE_FIELDS', tid, '已核对分类单元缺名称、归属、来源或正文条目定位')
        source_ok(t['source_id'], tid)
        claim_refs(t['evidence_ids'], tid)
        if expected_record_level == 'soil_genus':
            if t['profile_kind'] != 'not_required':
                issue('SOIL_PROFILE_SCOPE', tid, '市级土属条目应标记not_required，逐土种剖面由土种志单独核验')
        else:
            if t['profile_kind'] in {'missing', 'not_required'} or not t['profile_locator']:
                issue('SOIL_PROFILE', tid, '县级土种不能在缺剖面/历史记述依据时标为已核对')
            else:
        ...
        if t['profile_kind'] == 'third_survey':
            ...
            if not {'profile_photo', 'landscape_photo'} <= roles:
                issue('SOIL_PHOTO_ROLE', tid, '三普土种照片须明确profile_photo和landscape_photo角色，地图/表格不替代')
