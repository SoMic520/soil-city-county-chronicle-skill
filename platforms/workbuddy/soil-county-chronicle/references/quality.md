...
| 文件 | 用途与填写规则 |
|---|---|
| project.json | 项目范围、规范锁定、专题四态、六章状态、已完成的人工终检；不是县域事实库 |
...
| soil_types.csv | 按确认清单逐土种登记分类、名称、来源及原表定位、主张、剖面/历史记述来源、照片、正文位置和draft/verified/withheld状态；与soil_inventory核对 |
| gaps.csv | 具体缺口及影响章、对应要求、优先查哪份已有报告、最小补件、责任方、open/resolved、处理结果和依据 |
| length-plan.json | 模板身份/哈希/实测值、模板与本县结构清单、六章目标/上下限/实际值、正式例外和锁定状态；按length执行，不是法定全国字数表 |
| layout.json | schema版本2；styles/execution记录三普基准，county_requirements记录县级要求，逐字段例外、局部保护对象和12项实际格式检查分开登记；不再默认镜像页边距或弹性图表空行 |
...
requirements的15个基础映射项是本技能按N1归纳的编纂工作清单，不是官方发布的CSV或新的全国验收表。实际地方要求需追加，不删除基本内容映射来制造完整。covered必须定位正文，并关联对应主张或图件；not_applicable必须有适用依据和处置依据，不能因材料缺失就勾选省略。关联专题还须与project.topics状态一致。历史材料不足时先记open及具体缺口，允许有据部分起草；不用“无变化”填空。

soil_inventory.expected_ids来自实际已核对的完整分类清单，不由已经写完的条目倒推。soil_types的profile_kind为third_survey、historical_profile、historical_description或missing：三普剖面需至少分别关联剖面照和景观照；figures.kind分别登记profile_photo、landscape_photo，且照片须关联该土种的主张，地图和表格不能替代照片。historical_description可无照片，但有历史记述来源和位置。脚本检查照片ID数量、声明角色和关联；照片的真实身份、内容及全部亚类总体记述仍须人工核对。写完一个土种不能代表第三章已完成。
...
旧项目不重新init、不覆盖原台账。先备份，把缺少的requirements.csv、soil_types.csv、gaps.csv和length-plan.json从新模板补入；在project.json补建soil_inventory，并按已有材料逐项填实。旧evidence.csv暂缺depends_on、figures.csv暂缺kind、requirements.csv暂缺reuse_basis列仍可读取，在需要相应功能时追加该列。旧项目可继续局部起草，但正式终稿检查会要求补齐新增覆盖台账和篇幅契约。任何升级都不自动把记录置为verified/covered/done/locked。
...
具体计量、折算和阈值完整执行[length](length.md)。模板和终稿必须使用同一个measurement_version；模板各章值不能手填估计，须保存只读计量输出。第三至六章还须标出模板固定叙述范围和结构单元清单，登记模板结构数量、本县同口径数量和定位。模板固定叙述字符数应由实际标注区间汇总，不使用无来源的经验占比。
...
字体字号、章标题、缩进、粗体、页码、目录、题注、图表空行、表宽及分节逐项按[layout](layout.md)执行，创建/修改DOCX前必须完整读取。严格使用N1的“第一章—一、—（一）—1.—（1）”字体层级，不因引入章标题而把“一、”仍设Heading 1，也不把目录深度与正文可用标题层级混为一谈。
...
旧排版配置没有layout_schema_version=2时，资料整理可继续，但排版终稿检查要求先备份并升级。不得直接覆盖已有真实项目设置；逐项迁入新版，并核对差异。新模板的所有format_checks保持pending，不生成未经实际打开/渲染的passed记录。
...
最小交付包：可编辑志稿、按需求的PDF/附图册、资料及引用索引、数据/图表锁定记录、审查与版面记录、未解决事项和限制。源原图留存；是否交出原始敏感数据依权限。通用技能包交付不包含目标县真实资料或超大样稿。
