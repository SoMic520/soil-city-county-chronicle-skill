"""Measure and validate template-derived county soil chronicle length plans.
...
ADAPTATION_KINDS = {'3':'soil_types', '4':'property_indicators', '5':'evaluation_modules', '6':'recommendation_topics'}
...
def measure_docx(path):
    ...
    warnings = []
    if len(chapters) != 6:
        warnings.append(f'检测到{len(chapters)}个标题1章，不是县级导引的六章；须人工映射后才能锁定篇幅')
...
def validate_plan(plan, final=False):
    if not isinstance(plan, dict) or plan.get('length_schema_version') != 1:
    ...
    for key in ('soil_types', 'property_indicators', 'evaluation_modules', 'recommendation_topics'):
    ...
    for cid in CHAPTERS:
        ...
        if not isinstance(basis, str) or not basis.strip():
            issue('LENGTH_BASIS', cid, '缺模板值到本县目标值的适配依据')
    ...
    if final:
        ...
        if not isinstance(project.get('basis'), str) or not project['basis'].strip():
            issue('LENGTH_STRUCTURE', 'project_structure.basis', '终稿缺模板与本县结构计数清单定位')
...
def calculate_targets(plan):
    ...
    if not isinstance(plan, dict) or plan.get('length_schema_version') != 1 or plan.get('measurement_version') != VERSION:
